...
func trimThisLevel(tower []float64, level int) {
	fmt.Println(tower)
	for i := 0; i <= level; i++ {
		if tower[i] > 1 {
			tower[i] = tower[i] - 1
...
func normalizeTower(tower []float64) {
	for i := 1; i < len(tower); i++ {
		trimThisLevel(tower, i)
		preGlass := float64(0)
		for j := 0; j <= i; j++ {
			if j == i {
				tower[j] = preGlass / 2
			} else {
				tower[j], preGlass = (tower[j]+preGlass)/2, tower[j]
			}
		}
	}
}
...
func champagneTower(poured int, query_row int, query_glass int) float64 {
	tower := make([]float64, query_row+1, query_row+1)
	tower[0] = float64(poured)
	normalizeTower(tower)
	return math.Min(tower[query_glass], 1)
}
