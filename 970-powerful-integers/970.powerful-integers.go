...
func removeDuplicate(nums *[]int) {
	...
	*nums = (*nums)[:startIndex+1]
}

func powerfulIntegers(x int, y int, bound int) []int {
	xVal, yVal, boundVal = x, y, bound
	result := make([]int, 0)
	powerIter(&result, 0, 0, 1, 1)
	sort.Ints(result)
	if len(result) > 1 {
...
func powerIter(r *[]int, xPower int, yPower int, xPowerValue int, yPowerValue int) {
	value := xPowerValue + yPowerValue
	if value <= boundVal {
		*r = append(*r, value)
		if xVal != 1 {
			powerIter(r, xPower+1, yPower, xPowerValue*xVal, yPowerValue)
		}
		if yVal != 1 {
			powerIter(r, xPower, yPower+1, xPowerValue, yPowerValue*yVal)
		}
	}
}
