...
/*
 * @lc app=leetcode id=831 lang=golang
 *
...
func maskPII(S string) string {
	...
	for i, c := range S {
		if i == 0 {
			isEmail = c >= 'a' && c <= 'z' || (c >= 'A' && c <= 'Z')
			if isEmail {
				buffer.WriteByte(lower(S[0]))
				buffer.Write([]byte{'*', '*', '*', '*', '*'})
			}
		}
		if isEmail {
			if c == '@' {
				buffer.WriteByte(lower(S[i-1]))
				afterAt = true
			}
			if afterAt {
				buffer.WriteByte(lower(S[i]))
			}
		} else {
			if S[i] >= '0' && S[i] <= '9' {
	...
	if !isEmail {
		...
		if buffer.Len() > 10 {
			r.WriteByte('+')
			for i := 10; i < buffer.Len(); i++ {
				r.WriteByte('*')
			}
			r.WriteByte('-')
		}
		r.Write([]byte{'*', '*', '*', '-', '*', '*', '*', '-'})
		r.Write(buffer.Bytes()[buffer.Len()-4:])
		return r.String()
	} else {
